...
if (Compute_BW_stats_T_matrix):
    ...
    splitDir = c.IVECTOR_TEST_DIR.split('/')
    ...
    for idx, filename in enumerate(file_list):
        ...
        filenameSave = c.IVECTOR_TEST_DIR + '/' + filenameFolder + '/' + locFileData
        N, F = compute_bw_stats(feat_and_label['feat'], UBM);
        x = extract_ivector(np.append(N,F),UBM,T)
        if (not os.path.exists(c.IVECTOR_TEST_DIR + '/' + filenameFolder)):
            os.mkdir(c.IVECTOR_TEST_DIR + '/' + filenameFolder)
