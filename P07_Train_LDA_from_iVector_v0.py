...
if (ComputeNewModel):
    print("Definindo novos modelos...")
    if (os.path.exists(SpheringFileName)):
        os.remove(SpheringFileName)  
    if (os.path.exists(LDAFileName)):
        os.remove(LDAFileName)
    if (os.path.exists(PLDAFileName)):
        os.remove(PLDAFileName)
    if (os.path.exists(MTXFileName)):
        os.remove(MTXFileName)
    if (os.path.exists(TRAIN_THR_FileName)):
        os.remove(TRAIN_THR_FileName)
...
show_result = True
# n_classes = 104
file_list = find_feats(c.IVECTOR_TRAIN_DIR)
file_list.sort()
embeddings = {}
...
for i, file in enumerate(file_list):
    # rs = file.split('/')[-2]
    # idxLOC = re.findall(r'\d+',rs)
    # spk = int(idxLOC[0]) # filename: DIR_OF_FILE/L_####_U_####_.pth
